...
def _is_skipped(payload):
    """True for a branch the workflow did not take.

    Nodes that were bypassed still come back as populated one-element lists, e.g.
    `require_byok_api_key: [{"action": "skipped", "status": "skipped_by_condition"}]`
    on every BYOK run whose key was accepted. Treating such a marker as the
    result would report a rejected key on runs that never had a key problem.
    """
    return (payload.get("action") == "skipped"
            or payload.get("status") == "skipped_by_condition")


def _generator_payload(result_json):
    """Pick the terminal node payload from a /result body, in priority order.

    Returns (node_id, payload) so callers can tell WHICH terminal carrier the
    workflow exited through — `require_byok_api_key` in particular needs its own
    message, since that node can come back with no failure text of its own.
    """
    for key in (
        ...
        if isinstance(node, list) and node and isinstance(node[0], dict) \
                and not _is_skipped(node[0]):
            return key, node[0]

    # Fallback: a terminal node this build does not know by name — e.g. a new
    # partial-success carrier that ships geometry when validation could not be
    # completed. Anything carrying a model counts, so a backend that adds such a
    # node does not need a matching plugin release to be usable.
    for key, node in result_json.items():
        if not (isinstance(node, list) and node and isinstance(node[0], dict)):
            continue
        data = _unwrap(node[0])
        if data.get("model_url") or any(
                _artifact_url(data.get(k))
                for k in ("model", "model_artifact", "glb_artifact")):
            return key, node[0]
    return None, None


# Exit node of sketch_to_3d_byok_v2 when the caller's provider key is missing,
# malformed, or rejected by the provider.
BYOK_KEY_NODE = "require_byok_api_key"
BYOK_KEY_MESSAGE = (
    "Nova3D could not use your provider API key. Check the key in the add-on "
    "preferences (and that the account has credit), then try again.")
...
class ParsedResult:
    ...
    def __init__(self, *, glb_url=None, model_artifact=None, code_artifact=None,
                 ...
                 error_category=None, operation=None, source_node=None):
        self.source_node = source_node  # terminal node the result came from
...
def parse_result(result_json):
    ...
    node_id, payload = _generator_payload(result_json)
    if payload is None:
    ...
    # Text the backend actually supplied, before any of our own fallbacks.
    explicit_message = (
        (failure or {}).get("user_message") or (failure or {}).get("message")
        or data.get("user_message") or data.get("message")
    )

    # Exiting through the BYOK key node always means the run failed on the
    # caller's provider key — even when that node carries no text of its own, in
    # which case the generic "Generation failed. Try again." would send the user
    # looking in the wrong place.
    byok_key_rejected = node_id == BYOK_KEY_NODE and not glb_url
    if byok_key_rejected:
        has_failure = True

    if explicit_message:
        message = explicit_message
    elif byok_key_rejected:
        message = BYOK_KEY_MESSAGE
    elif has_failure:
        message = friendly_message(category)
    else:
        message = None
    ...
    return ParsedResult(
        ...
        operation=data.get("operation"), source_node=node_id,
    )
