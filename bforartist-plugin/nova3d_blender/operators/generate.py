...
# Handle to the currently running job, so the stop-watching operator can reach it.
_active_job = None
...
class _JobModalMixin:
    ...
    def _start_jobs(self, context, factories):
        ...
        # 10 fps: fast enough for the running indicator to rotate smoothly, and
        # it doubles as the event-pump tick (draining the worker queue more
        # often is harmless — it is usually empty).
    ...
    def _start_next(self, context):
        ...
        wm.nova3d_outcome = ""      # clear the previous run's success/failure styling
        self._job.start()

    def modal(self, context, event):
        # ESC deliberately does NOT abort. The backend cannot be cancelled, so
        # bailing out would only lose track of a run that keeps going and is
        # billed regardless; it is left to finish and its result collected.
        if event.type != "TIMER":
            return {"PASS_THROUGH"}
        while True:
            try:
            ...
            if result is not None:
                return result
        # Repaint on every tick, not just when the worker reports something:
        # /status is only polled every few seconds, so without this the running
        # indicator would sit frozen between updates.
        _tag_redraw(context)
        return {"PASS_THROUGH"}
    ...
    def _import_asset(self, context, payload):
        ...
        # A partial success (e.g. delivered without the validation pass) still
        # imports normally, but must not read as a clean run — the warning
        # replaces the status line and is raised in the info bar too.
        warning = payload.get("warning")
        if warning:
            wm.nova3d_status = warning
            wm.nova3d_outcome = "warn"
            self.report({"WARNING"}, warning)
        else:
            wm.nova3d_outcome = "ok"
        _tag_redraw(context)
...
class NOVA3D_OT_generate(bpy.types.Operator, _JobModalMixin):
    ...
    bl_description = ("Generate a 3D model from your prompt, using Nova3D credits "
                      "or your own provider key depending on the selected model")
    ...
    def invoke(self, context, event):
        ...
        # Which section the user last picked a model in decides how this run is
        # billed. Resolved here on the main thread, along with the provider key
        # a BYOK run needs, so a misconfigured choice fails instantly.
        if scene.nova3d_billing == "byok":
            option = constants.model_option_by_id(scene.nova3d_byok_model)
            if not constants.is_byok_option(option):
                # The BYOK dropdown is showing its "no key added yet"
                # placeholder — there is nothing to run.
                self.report({"ERROR"},
                            "Paste a provider API key first, or pick a model "
                            "under Nova3D Credits.")
                return {"CANCELLED"}
        else:
            option = constants.model_option_by_id(scene.nova3d_paid_model)

        provider = option[5]
        byok_api_key = byok_key_for(prefs, provider) if provider else ""
        if provider and not byok_api_key:
            label = constants.byok_provider_label(provider)
            self.report({"ERROR"},
                        f"This model runs on your own {label} key, which is no "
                        "longer saved. Paste it again, or pick a credits model.")
            return {"CANCELLED"}
...
class NOVA3D_OT_stop_watching(bpy.types.Operator):
    """Stop tracking the running generation WITHOUT stopping it.

    Not exposed in the panel — the backend has no cancel endpoint, so there is
    nothing a button here could actually stop. Kept as an escape hatch: the run
    stays in the pending list and Resume collects its result later.
    """

    bl_idname = "nova3d.stop_watching"
    bl_label = "Stop Watching Generation"
    bl_description = ("Stop tracking the running generation. It keeps running on "
                      "the server and is still billed — use Resume to collect it")

    def execute(self, context):
        if _active_job is not None:
            _active_job.cancel()
            context.window_manager.nova3d_status = (
                "Stopped watching. The run continues — use Resume to collect it.")
            self.report({"INFO"},
                        "No longer tracking this run; it continues on the server.")
            return {"FINISHED"}
