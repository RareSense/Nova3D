...
def build_meta(*, prompt, model_option, credits, workflow_id, conversation_id,
               message_id, parsed, files, uv_summary=None, workflow_name=None):
    """Assemble the meta.json pointer record."""
    option_id, label, tier, display_credits, _badge, provider = \
        constants.normalize_model_option(model_option)
    return {
        "schema_version": 1,
        ...
        "model": {
            ...
            # "" for paid-credit runs; the provider id for BYOK runs. The key
            # itself is deliberately never written to disk.
            "code_llm_provider": provider,
            "billing": "byok" if provider else "credits",
        },
        "credits": {
        ...
        "workflow": {
            "name": workflow_name or constants.WORKFLOW_NAME,
