...
Powered by the hosted Nova3D service; billed in Nova3D credits, or on your own
...
- **BYOK (bring your own key)** — models listed as *"— Gemini key"*, *"— Anthropic
  key"*, etc. run on your own provider API key instead and cost **0 credits**.
  Paste the key under **Provider Keys** in the add-on preferences; the model
  dropdown tells you whether the key for the selected model is set. Those runs
  are billed by the provider, so keep credit available on that account.
